...
import { LayoutDashboard, ScanLine, LogOut, Train, User } from 'lucide-react';
    ...
    <div className="flex h-screen bg-slate-50/50">
      ...
      <div className="w-64 bg-white border-r border-gray-100 flex flex-col shadow-sm z-10">
        <div className="p-6 pb-2 border-b border-gray-100/50">
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-blue-600 text-xs font-semibold tracking-wide">OFFICER PORTAL</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-1">
          ...
          <NavLink to="/officer/scan" className={navClass}>
            <ScanLine size={20} />
            Scan & Process
          </NavLink>
          <NavLink to="/officer/profile" className={navClass}>
            <User size={20} />
            My Profile
          </NavLink>
        </div>
        ...
        <div className="p-4 border-t border-gray-100">
          <Link to="/officer/profile" className="flex items-center gap-3 px-4 py-3 mb-2 hover:bg-gray-50 rounded-xl transition-colors cursor-pointer">
            <div className="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-blue-600 font-bold text-sm">
              {user?.fullName?.charAt(0) || 'O'}
            </div>
            <div className="overflow-hidden">
              <p className="text-sm font-semibold text-gray-800 truncate">{user?.fullName}</p>
              <p className="text-xs text-gray-500 truncate">Station Officer</p>
            </div>
          </Link>
          <button 
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden bg-[#f8fafc]">
        <div className="h-16 bg-white border-b border-gray-100 flex items-center px-8 shadow-sm z-0">
          <h2 className="font-semibold text-gray-800">Station Officer Workspace</h2>
        </div>
